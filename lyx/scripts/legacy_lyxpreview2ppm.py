...
#this funtion is identical to the commented one above, but inject \pagecolor{lyxbg} in the appropriate place for xelatex

def legacy_latex_file(latex_file, fg_color, bg_color):
    use_polyglossia_re = re.compile(b"\\s*\\\\usepackage{polyglossia}")
    use_preview_re = re.compile(b"\\s*\\\\usepackage\\[([^]]+)\\]{preview}")
    begin_preview_re = re.compile(b"\s*\\\\begin{preview}")
    fg_color_gr = make_texcolor(fg_color, True)
    bg_color_gr = make_texcolor(bg_color, True)
    ...
    #                 polyglossia = True
    #                 continue
            if match == None and not polymatch == None:
                    new_line = b"""
    \\usepackage{color}
    \\definecolor{lyxfg}{rgb}{%s}
    \\definecolor{lyxbg}{rgb}{%s}
...
\\makeatother
""" % previewopts
        elif not added_pagecolor:
            match = begin_preview_re.match(line)
            if match:             
                new_line = begin_preview_re.sub(b"\\\\begin{preview}\\\\pagecolor{lyxbg}", line)
                added_pagecolor = 1
        tmp.write(new_line)
    if added_preamble and added_pagecolor:
        copyfileobj(tmp, open(latex_file,"wb"), 1)

    return added_preamble
